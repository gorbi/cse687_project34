...
std::shared_ptr<AbstractXmlElement> XmlProcessing::makeTaggedElementNText(const std::string& tag, const std::string& value)
{
	std::shared_ptr<AbstractXmlElement> ptr(new TaggedElement(tag));
	std::shared_ptr<AbstractXmlElement> ptrTxt(new TextElement(value));
	ptr->addChild(ptrTxt);
	return ptr;
}
